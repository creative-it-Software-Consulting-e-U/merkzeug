...
function App() {
  ...
  return <div className={`ide-app${previewEnabled && previewCss ? ' template-live' : ''}`}>
    ...
    {extras && <aside className="editor-extras"><TemplateLocation value={templateState.assigned} templates={templateState.templates} onChange={async name => { setTemplateState(await request('templateAssign', { name: name ?? '' })); await refreshTemplatePreview() }} /><div className="editor-extra-actions"><AboutButton openUrl={url => { void request("open", { href: url }).catch(e => setError(String(e))) }} /><ReleaseNotesButton /><button onClick={() => void request('settings').then(refreshTemplatePreview).catch(e => setError(String(e)))}>{translate("PDF template …")}</button><label className="template-preview-toggle"><input type="checkbox" checked={previewEnabled} onChange={e => void toggleTemplatePreview(e.target.checked)} /> {translate('Use PDF template while editing')}</label>{info && import.meta.env.VITE_MERKZEUG_TOURS !== 'disabled' && <><GuidedTour edition="intellij" seen={true} onSeen={() => void request('tourSeen')} /></>}</div>{previewEnabled && !previewCss && <p>{translate('Assign a PDF template in Settings to preview its content styles.')}</p>}{info && <VaultGuidance vaultId={info.vault} host={guidanceHost} />}</aside>}
