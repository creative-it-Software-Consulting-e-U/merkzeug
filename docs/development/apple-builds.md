...
The local iOS development archive and App Store distribution IPA export both succeeded with Xcode 27 beta. The exported app's signature was verified, with `get-task-allow=false` and `beta-reports-active=true`. This is not evidence of a successful upload or TestFlight processing. The owner authorized Xcode Cloud access to `creative-it-Software-Consulting-e-U/merkzeug`; the Xcode setup assistant reports that primary repository as **Connected**. The assistant currently blocks progress on the public `ionic-team/capacitor-swift-pm` dependency and incorrectly directs setup to that third-party organization. Public Swift packages should require no separate authorization according to [Apple's dependency documentation](https://developer.apple.com/documentation/xcode/making-dependencies-available-to-xcode-cloud). No request was sent to that organization, and no workflow/build has been created yet. Rechecking the assistant did not clear the dependency issue. The source changes also still need a reviewed Git revision before the first cloud build.
