...
## Combined TestFlight candidate, 13 September 2026

The open feature and dependency PRs were integrated together for TestFlight testing. Capacitor core/CLI/iOS are aligned to 8.5.1; the local Swift package retains the official [8.5.1 binary URLs and checksums](https://github.com/ionic-team/capacitor-swift-pm/blob/8.5.1/Package.swift). Electron is 44.2.0. Mobile and IntelliJ web builds use Vite 8 with React plugin 6; Electron keeps Vite 7 and React plugin 5 because electron-vite 5 declares Vite 5–7 support. The combined lockfile is regenerated and tested from `npm ci`.

The live Cloud workflows were found set to manual starts on the former roadmap branch, superseding the automatic trigger setup recorded above. Both manual branch selectors now target `main`; TestFlight candidates are explicitly started through the API after the combined main push. Existing signing and distribution actions are preserved. No App Review submission is part of this operation.

Cloud build 67 exposed an Electron 44 packaging requirement: when Cloud skips the local Electron binary, electron-builder must receive an exact version. The desktop manifest now pins `44.2.0` instead of a range; the lockfile retains the same runtime version.
