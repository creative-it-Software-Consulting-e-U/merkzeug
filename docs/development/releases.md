...
The canonical source license is the root `LICENSE` (MIT). `node scripts/build-licenses.mjs` generates the localized license views in `resources/legal/EULA.*.md` and the website; those legacy filenames are retained for packaging compatibility. The IntelliJ affiliation note is informational and adds no license restrictions. Preserve third-party notices. Historical releases keep their original notices; do not replace their binaries or license files. See [public-release preparation](public-release-preparation.md) before making repository history public. Preparation does not authorize repository visibility changes, store uploads or a website deployment.
