...
import type { CalendarEvent, CalendarPerson } from './calendar'
const slugifyTitle = (title: string): string => title.toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '-').replace(/^-+|-+$/g, '')
...
export function meetingNoteContent(ev: CalendarEvent): string {
  ...
  lines.push(`calendar-event: ${yamlQuote(ev.id)}`, `calendar-start: ${yamlQuote(ev.start)}`)
