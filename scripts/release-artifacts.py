...
def assemble(directory: Path, complete=False):
    version = check()
    ...
    if complete:
        expected = {f'Merkzeug-{version}-mac-{arch}.dmg' for arch in ['x64','arm64']}
        expected |= {f'Merkzeug-{version}-win-{arch}.exe' for arch in ['x64','arm64']}
        expected |= {f'Merkzeug-{version}-linux-x64.{ext}' for ext in ['AppImage','deb','rpm']}
        expected |= {f'merkzeug-{version}.zip', docs.name}
    ...
    manifest = {'version':version,'sourceCommit':commit,'workingTreeModified':dirty,
                'workflowRun':os.environ.get('GITHUB_RUN_ID'),'completeMatrix':complete,'artifacts':entries,
...
Preview editions: desktop (macOS, Windows, Linux) and IntelliJ IDEA.
...
if __name__ == '__main__':
    ...
    try: assemble(args.directory.resolve(),args.complete)
