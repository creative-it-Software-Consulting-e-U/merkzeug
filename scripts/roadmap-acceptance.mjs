...
try {
 ...
 const meetings = (await readdir(vault)).filter(name => /^Synthetic calendar meeting\.md$/.test(name))
 ...
 assert.equal((await readdir(vault)).filter(name => /^Synthetic calendar meeting\.md$/.test(name)).length, 1)
